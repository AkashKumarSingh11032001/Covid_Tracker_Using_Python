...
def CreateWidgets():
    worldwideActiveLabel = Label(
        root, text="WORLDWIDE ACTIVE CASES : ", bg="thistle4")
    ...
    worldwideConfirmedLabel = Label(
        root, text="WORLDWIDE CONFIRMED CASES : ", bg="thistle4")
    ...
    worldwideRecoveredLabel = Label(
        root, text="WORLDWIDE RECOVERED COUNT : ", bg="thistle4")
    ...
    worldwideDeathsLabel = Label(
        root, text="WORDWIDE DEATHS COUNT : ", bg="thistle4")
    ...
    worldwideActiveEntry.config(text=str(Covid().get_total_active_cases()))
    worldwideConfirmedEntry.config(
        text=str(Covid().get_total_confirmed_cases()))
    worldwideRecoveredEntry.config(text=str(Covid().get_total_recovered()))
    worldwideDeathsEntry.config(text=str(Covid().get_total_deaths()))
    ...
    root.countriesComboBox = Combobox(
        root, width=18, values=countryName, state="readonly")
    root.countriesComboBox.grid(row=4, column=1, padx=5, pady=2)

    findButton = Button(root, text="FIND", command=findDetails, bg="red")
    findButton.grid(row=5, column=1, padx=5, pady=5, columnspan=2)

    activeCasesLabel = Label(
        root, text="TOTAL ACTIVE CASES :       ", bg="thistle4")
    ...
    confirmedCasesLabel = Label(
        root, text="TOTAL CONFIRMED CASES :    ", bg="thistle4")
    ...
    recoveredCasesLabel = Label(
        root, text="TOTAL RECOVERED COUNT :    ", bg="thistle4")
    ...
    deathsLabel = Label(
        root, text="TOTAL DEATHS COUNT :       ", bg="thistle4")
...
def findDetails():
    ...
    data_updated_at = dt.fromtimestamp(
        updated_time_epoch/1000).strftime("%d-%m-%Y %H:%M:%S")
...
root.title("Covid_Track")
# root.config(background="black")

# #background Img
# photo = PhotoImage(file = "A:\\Project\\Covid_Tracker_Using_Python\\covid.jpg")
# canvas1 = Canvas(root, width=40,height=400)
